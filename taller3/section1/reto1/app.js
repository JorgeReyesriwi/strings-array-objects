...
function getUsername(name, lastname) {
  let threeWordsName = name.slice(0, 3);
  let threeWordsLasname = lastname.slice(0, 3);
  let username = threeWordsName + threeWordsLasname;
  let usernameFinal = username + contador;
  let contador = 1;
  while (users[usernameFinal]) {
...
function getEmail(usernameFinal, domain) {
  email = usernameFinal + domain
  return email
}
